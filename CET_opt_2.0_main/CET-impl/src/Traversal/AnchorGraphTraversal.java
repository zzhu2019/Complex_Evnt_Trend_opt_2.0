...
import src.util.CustomDS.LinkedListNode;
import src.util.CustomDS.CustomObjLinkedList;
...
public class AnchorGraphTraversal extends GraphTraversal {
    ...
    HashMap<Integer, CustomObjLinkedList> anchorPaths;
    // for start nodes which are also anchor nodes, they have a fine-grained DS to store sub-paths
    HashMap<Integer, HashMap<Integer, CustomObjLinkedList>> anchorPathsForStartNodes;
    ...
    private void initMap() {
        for(Integer anchorNode : anchorNodes) {
            if(graph.startContains(anchorNode)) {
                anchorPathsForStartNodes.put(anchorNode, new HashMap<Integer, CustomObjLinkedList>());
            }
            else {
                anchorPaths.put(anchorNode, new CustomObjLinkedList());
            }
        }
    }
...
//        System.out.println("start node "+start);
        ...
        if((isAnchor[s] && curStack.size()>1) || graph.endContains(s)) {
            ...
            if(graph.startContains(curStack.getFirstElement())) {
                ...
                anchorPathsForStartNodes.get(curStack.getFirstElement()).
                        putIfAbsent(curStack.peek(), new CustomObjLinkedList());
                anchorPathsForStartNodes.get(curStack.getFirstElement()).
                        get(curStack.peek()).addAtHead(curStack.getAllElements());
            }
            else {
                // for other anchor nodes
                anchorPaths.get(curStack.getFirstElement()).addAtHead(curStack.getAllElements());
            }
            return;
        }
    ...
    void concatenate(int start) {
        if(concatenateType.equals(ConcatenateType.BFS)) {
            ...
            HashMap<Integer, CustomObjLinkedList> map = anchorPathsForStartNodes.get(start);

            for(Integer endNode : map.keySet()) {
                boolean isFirstConcatenate = true;
                CustomObjStack<int[]> restPaths = new CustomObjStack<>();

                LinkedListNode itr = map.get(endNode).getHead();
//                Iterator<int[]> itr = map.get(endNode).iterator();
                while(itr != null) {
                    int[] startPath = itr.getData();
...
//                }
    ...
    void BFSSubConcatenate(int start) {
        ArrayQueue<CustomObjLinkedList> queue = new ArrayQueue<>(graph.getStartPointNum());
        ...
        while(!queue.isEmpty()) {
            ...
            CustomObjLinkedList currentPaths = queue.poll();
            // For each sub-path
            LinkedListNode itr = currentPaths.getHead();
//            Iterator<int[]> itr = currentPaths.iterator();
            while(itr != null) {
                int[] subPath = itr.getData();
                ...
                CustomObjLinkedList combo = new CustomObjLinkedList();

                // Get all these sub-paths under the end node of the current sub-path
                LinkedListNode innerItr = anchorPaths.get(subPath[subPath.length - 1]).getHead();
//                Iterator<int[]> innerItr = anchorPaths.get(subPath[subPath.length - 1]).iterator();
                while(innerItr != null) {
                    int[] nextList = innerItr.getData();
                    ...
                    combo.addAtHead(newPath);
                    innerItr = innerItr.getNext();
                }
                queue.offer(combo);
                itr = itr.getNext();
            }
    ...
    void DFSSubConcatenate(int[] s, FixedSizeStack<int[]> curStack, CustomObjStack<int[]> restPaths) {
        ...
        LinkedListNode itr = anchorPaths.get(s[s.length - 1]).getHead();
//        Iterator<int[]> itr = anchorPaths.get(s[s.length - 1]).iterator();
        while(itr != null) {
            int[] nextAnchorPath = itr.getData();
